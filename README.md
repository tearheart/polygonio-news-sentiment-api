# News Sentiment API (API Container)
...
Clone the GitHub repository `polygonio-news-sentiment-api`.
``` bash
git clone https://github.com/tearheart/polygonio-news-sentiment-api
```

Move into new directory `polygonio-news-sentiment-api`.
``` bash
cd polygonio-news-sentiment-api
```
...
Access to the API interface, the output of `make local-run` should output a link like `http://0.0.0.0:8160` or `http://localhost:8160`.  Click on the link and explore the API interface.  Click on the `sentiment` link, then click `Try Out` and you will be able to test the FinBERT sentiment model.

The productionized API endpoint lives at `http://0.0.0.0:8160/v1/reference/sentiment` (full payload) or you can filter by stock ticker at `http://0.0.0.0:8160/v1/reference/sentiment?ticker=<insert your favorit stock ticker>` (i.e. ticker=AAPL).
